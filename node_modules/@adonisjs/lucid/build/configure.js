...
export async function configure(command) {
    ...
    if (dialect === undefined) {
        dialect = await command.prompt.choice('Select the database you want to use', Object.keys(DIALECTS), {
            validate(value) {
                return !!value;
            },
        });
    ...
    if (dialect in DIALECTS === false) {
        command.error(`The selected database "${dialect}" is invalid. Select one from: ${string.sentence(Object.keys(DIALECTS))}`);
        command.exitCode = 1;
        return;
    }
    /**
    ...
    if (shouldInstallPackages === undefined) {
        shouldInstallPackages = await command.prompt.confirm('Do you want to install additional packages required by "@adonisjs/lucid"?');
    }
